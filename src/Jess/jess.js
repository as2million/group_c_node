...
router.get("/bento", (req, res) => {
  db.query("SELECT * FROM `product` WHERE 1").then(([results]) => {
    res.json(results);
  });
});
...
router.get("/message", (req, res) => {
  db.query("SELECT * FROM `message` WHERE 1").then(([results]) => {
    res.json(results);
  });
});
...
router.get("/productSid", (req, res) => {
  db.query("SELECT * FROM `message` WHERE product_sid=1").then(([results]) => {
    res.json(results);
  });
});
...
router.get("/member", (req, res) => {
  db.query(
    "SELECT member_list.name,message.content,message.product_sid FROM `member_list` INNER JOIN message on  member_list.member_sid=message.member_sid WHERE product_sid=1"
  ).then(([results]) => {
    res.json(results);
  });
});
